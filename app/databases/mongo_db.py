...
async def get_entries_with_in(
    ...
    """
    Get entries where a field value is in a list of values
    Example: Get all users with ids in ["abc", "def"]
    """
    if not values:
        return []
    ...
    return await self.get_entries(
        ...
        doc_filter={field_name: {"$in": values}},
        collection_name=collection_name
    )


async def get_entries_dict_with_in(
        self,
        collection_name: str,
        field_name: str,
        values: List[Any],
    ...
    """
    Get raw dictionaries where a field value is in a list of values
    Example: Get all documents in 'products' collection with prices in [100, 200]
    """
    if not values:
        return []

    return await self.get_entries_dict(
        collection_name=collection_name,
        doc_filter={field_name: {"$in": values}}
    )
